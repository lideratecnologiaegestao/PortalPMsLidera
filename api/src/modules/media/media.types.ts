...
// ─────────────────────────────────────────────────── headers de segurança SVG

/**
 * Aplica os headers de segurança obrigatórios para respostas SVG.
 *
 * Controles aplicados:
 * - `X-Content-Type-Options: nosniff` — impede sniffing de MIME pelo browser.
 * - `Content-Security-Policy: default-src 'none'; sandbox` — política restritiva:
 *   sem scripts, sem estilos externos, sem recursos de rede. O atributo `sandbox`
 *   desativa execução de JS mesmo que algum script escape a sanitização.
 *   NÃO inclui `style-src 'unsafe-inline'` nem `img-src data:`.
 * - `Content-Disposition: attachment; filename="<nomeArquivo>.svg"` — força download
 *   em navegação top-level (o controle mais forte). Em subrecursos (<img src>,
 *   <object>, <embed>) o header é ignorado pelo browser, portanto logos e ícones
 *   embutidos em páginas continuam funcionando normalmente.
 *
 * @param res         Objeto Response do Express.
 * @param nomeArquivo Nome base do arquivo SEM extensão (ex.: "brasao-municipal").
 */
export function aplicarHeadersSvgSeguro(res: Response, nomeArquivo: string): void {
  res.setHeader('X-Content-Type-Options', 'nosniff');
  res.setHeader('Content-Security-Policy', "default-src 'none'; sandbox");
  // Sanitiza o nome para uso no header: remove aspas e caracteres de controle
  const nomeSanitizado = nomeArquivo.replace(/["\r\n]/g, '').slice(0, 200) || 'arquivo';
  res.setHeader('Content-Disposition', `attachment; filename="${nomeSanitizado}.svg"`);
}
