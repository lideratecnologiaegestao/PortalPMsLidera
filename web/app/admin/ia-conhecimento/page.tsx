...
/**
 * Base de Conhecimento do Assistente de IA.
 * Roles: GESTOR / ADMIN_PREFEITURA (verificado no backend).
...
interface FonteContagem {
  fonte: string;
  // O backend retorna `chunks` (e `ultimoCriado`). Mantemos o contrato real.
  chunks: number;
  ...
  async function reindexar() {
    ...
    try {
      ...
      if (res.enfileirado) {
        ...
        // Recarrega o status após ~5s para refletir o novo estado
        setTimeout(() => {
          carregarStatus();
        }, 5000);
      } else {
      ...
      // Desabilita o botão por 6s mesmo em caso de erro para evitar spam
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-fg/60">
            ...
            notícias, secretarias, documentos) e esta base de conhecimento para
            o assistente entender perguntas por significado, não só por
            palavras.
          </p>
        </div>
        <button
            ...
            {(status.porFonte?.length ?? 0) > 0 && (
              <div className="overflow-x-auto">
                ...
                >
                  <thead>
                    <tr>
                      <th className={`${ui.th} w-3/4`} scope="col">
                        Fonte
                      </th>
                      <th
                        className={`${ui.th} text-right`}
                        scope="col"
                      >
                        Trechos
                      </th>
...
/**
 * O usuário pode:
 *  - digitar uma tag e pressionar Enter, vírgula ou Tab → adiciona como chip
 *  - clicar no × de um chip → remove
 * O input nunca limpa o foco ou o cursor enquanto o modal está aberto.
 */
function TagsEditor({
  value,
  onChange,
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === 'Enter' || e.key === ',' || e.key === 'Tab') {
      ...
      // Remove a última tag ao apertar Backspace com input vazio
  ...
  function remover(tag: string) {
    ...
    // Devolve o foco ao input após remover
...
function ModalFormulario({
  ...
  open: boolean;
  item: ItemConhecimento | null; // null = novo
  onClose: () => void;
  onSalvo: () => void;
}) {
  const idBase = useId();
  // Estado do formulário — nunca lido via closure no submit; lemos via ref.
  const [form, setFormRaw] = useState<FormState>(FORM_VAZIO);
  const formRef = useRef<FormState>(FORM_VAZIO);

  // Atualiza ref sempre que o estado muda (para o submit usar sem stale closure).
  ...
  // Preenche form quando o modal abre
  useEffect(() => {
    if (!open) return;
...
export default function IaConhecimentoPage() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      // Fixados primeiro, depois por data de atualização mais recente
  ...
  // Filtro local — busca em pergunta, resposta e tags
  ...
  return (
    <div className="space-y-5">
      {/* Cabeçalho */}
      <AdminHeader
        title="Base de Conhecimento do Assistente de IA"
        description="Ensine o assistente virtual. Cadastre perguntas frequentes e respostas oficiais — o bot prioriza estas respostas. Marque como Fixado os fatos essenciais (sempre considerados)."
      >
