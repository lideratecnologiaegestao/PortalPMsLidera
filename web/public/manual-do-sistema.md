...
**O que é:** página inicial do painel admin, que exibe o índice de conformidade PNTP (Programa Nacional de Transparência Pública) do município.
...
- Índice percentual de conformidade PNTP e o selo atual (Elevado, Prata, Ouro ou Diamante).
- Alerta vermelho com os critérios essenciais pendentes que bloqueiam a obtenção do próximo selo.
- Barra de progresso por dimensão (com peso e percentual atingido).
...
**O que é:** gerencia janelas de aviso (popups) que aparecem para o visitante ao acessar o portal.
...
**O que é:** motor de publicação de documentos oficiais (Leis, Decretos, Portarias, Resoluções, Relatórios, etc.) com tipos e taxonomias definidas pelo TCE-MT.
...
**Como chegar:** menu lateral > Transparência > **Transparência**.
...
- Os dados do APLIC/TCE-MT alimentam automaticamente a Transparência quando importados pelo módulo Contas (APLIC).
...
**Como chegar:** menu lateral > Transparência > **Contas (APLIC)**.
...
- A IA fiscal usa esses dados para responder perguntas como "qual o total pago ao fornecedor X em 2025" — use consulta estruturada, não embeddings.
- Cada carga importada substitui os dados do mesmo exercício/competência. Não há sobreposição — a carga mais recente prevalece para o período.

---

## Páginas (CMS / Construtor)
...
- Informar a instância do WhatsApp (Evolution API).
...
- Se o chat estiver vinculado a uma manifestação de ouvidoria (via bot), o protocolo da manifestação aparece nas informações da conversa. Ouvidores com acesso podem clicar no link para ir ao painel da ouvidoria.
...
- Buraco na via, Terreno abandonado, Animal abandonado, Iluminação pública, Lixo/entulho, Poda de árvore, Sinalização, Outro.
...
**O que é:** dashboard de conformidade com a LGPD, exibindo indicadores gerais sobre solicitações de titulares, incidentes de segurança e estado do Encarregado (DPO).
...
| PNTP | Programa Nacional de Transparência Pública — avalia conformidade do portal. |
